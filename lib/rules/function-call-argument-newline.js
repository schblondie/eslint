...
module.exports = {
    ...
    create(context) {
      const sourceCode = context.sourceCode
         ...
         */
        function check(node) {
          const minArguments = context.options[0]?.minArguments ?? 2;

          if (node.arguments.length < minArguments) {
            if (context.options[0]?.minArguments) checkArguments(node, checkers.unexpected);
            return;
            }
